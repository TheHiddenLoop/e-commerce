...
export function Navbar({ onClick }) {
  ...
  return (
    <div
      className={`fixed top-0 left-0 right-0 z-[500] bg-bgGlass backdrop-blur-[20px] border-b border-b-border h-16 ${
    ...
    >
      <div className="flex items-center justify-between h-full px-2 md:px-[80px]">
        <div className="flex items-center gap-2 md:gap-4 h-full">
          {auth && (
            <div
              className="cursor-pointer font-bold text-primary md:hidden"
              onClick={onClick}
            >
              <MenuIcon size={24} />
            </div>
          ...
          <div className="flex items-start gap-10">
            <ul className="gap-6 items-center hidden md:flex h-full">
              {navItems.map((item, idx) => (
                <li key={idx} className="cursor-pointer px-2 py-1">
                  <a
            ...
            <div className="flex items-center gap-4 h-full">
              <div className="relative cursor-pointer flex items-center h-full">
                ...
                <span
                  className={`absolute -top-2 -right-2 w-5 h-5 bg-primary text-white rounded-full flex items-center justify-center text-xs font-bold shadow-lg ${
                    counts > 0 ? "flex" : "hidden"
                  }`}
                >
                  {counts}
                </span>
              ...
              <div
                ...
                className="flex items-center border-primary border-[3px] w-10 h-10 rounded-full justify-center cursor-pointer overflow-hidden"
              >
                <img
                  ...
                  <div className="flex flex-col gap-1">
                    {menuItems.map((item, i) => (
                      <Link
                        ...
                        onClick={() =>
                          (item.label === "logout" &&
                            handleLogout(item.label)) ||
                          (item.label === "theme" && toggleDarkMode())
                        }
                        className="
